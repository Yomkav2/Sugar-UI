...
-- ======================
-- Preset Themes (обновленные для более яркого и современного вида)
-- ======================
SugarUI.Presets = {
    Pinky = {
        Background = Color3.fromRGB(30, 15, 25),
        Panel = Color3.fromRGB(40, 20, 35),
        Accent = Color3.fromRGB(255, 105, 180),
        AccentSoft = Color3.fromRGB(255, 150, 210),
        AccentDark = Color3.fromRGB(200, 60, 120),
        Text = Color3.fromRGB(250, 245, 255),
        Muted = Color3.fromRGB(190, 170, 180),
        ...
        Toggle = Color3.fromRGB(35, 15, 28),
        ToggleBox = Color3.fromRGB(240, 240, 240),
        Button = Color3.fromRGB(35, 15, 28),
        ButtonHover = Color3.fromRGB(50, 25, 40),
    },
    Amethyst = {
        Background = Color3.fromRGB(20, 15, 35),
        Panel = Color3.fromRGB(30, 25, 50),
        Accent = Color3.fromRGB(160, 110, 210),
        AccentSoft = Color3.fromRGB(180, 150, 220),
        AccentDark = Color3.fromRGB(100, 70, 130),
        Text = Color3.fromRGB(245, 245, 255),
        Muted = Color3.fromRGB(170, 160, 180),
        Shadow = Color3.fromRGB(0, 0, 0),
        Border = Color3.fromRGB(55, 45, 65),
        Highlight = Color3.fromRGB(255, 255, 255),
        Success = Color3.fromRGB(100, 200, 100),
        Warning = Color3.fromRGB(255, 200, 50),
        Error = Color3.fromRGB(255, 80, 80),
        Toggle = Color3.fromRGB(28, 22, 40),
        ToggleBox = Color3.fromRGB(210, 210, 210),
        Button = Color3.fromRGB(28, 22, 40),
        ButtonHover = Color3.fromRGB(48, 40, 65),
    },
    Dark = {
        Background = Color3.fromRGB(15, 15, 15),
        Panel = Color3.fromRGB(25, 25, 25),
        Accent = Color3.fromRGB(80, 160, 240),
        AccentSoft = Color3.fromRGB(50, 130, 220),
        AccentDark = Color3.fromRGB(0, 100, 170),
        ...
        ToggleBox = Color3.fromRGB(190, 190, 190),
    ...
    White = {
        ...
        Accent = Color3.fromRGB(50, 130, 210),
        AccentSoft = Color3.fromRGB(90, 150, 230),
        AccentDark = Color3.fromRGB(20, 80, 150),
        ...
        Warning = Color3.fromRGB(255, 200, 50),
        Error = Color3.fromRGB(255, 80, 80),
        Toggle = Color3.fromRGB(250, 250, 250),
        ToggleBox = Color3.fromRGB(50, 50, 50),
...
-- ======================
-- Вспомогательные (добавлены градиенты и улучшенные эффекты)
-- ======================
...
function SugarUI.Tween(instance, props, duration, style, dir)
    ...
    local tweenInfo = TweenInfo.new(duration or 0.28, style, dir)
...
function SugarUI.AddGradient(frame, rotation, colorSeq, transSeq)
    local gradient = Instance.new("UIGradient")
    gradient.Rotation = rotation or 90
    gradient.Color = colorSeq or ColorSequence.new(SugarUI.Theme.Panel, SugarUI.Theme.Background)
    gradient.Transparency = transSeq or NumberSequence.new(0, 0.1)
    gradient.Parent = frame
    return gradient
end
-- ======================
-- Button component (улучшенный: градиент, ripple с scale, hover scale)
-- ======================
...
function ButtonComponent.new(parent, text, callback)
    ...
    SugarUI.AddGradient(Btn, 90, ColorSequence.new(SugarUI.Theme.Button, SugarUI.Theme.ButtonHover))
    Btn.MouseEnter:Connect(function()
        SugarUI.Tween(Btn, {BackgroundColor3 = SugarUI.Theme.ButtonHover, Size = UDim2.new(1, -8, 0, 40)}, 0.18)
    end)
    Btn.MouseLeave:Connect(function()
        SugarUI.Tween(Btn, {BackgroundColor3 = SugarUI.Theme.Button, Size = UDim2.new(1, -10, 0, 38)}, 0.18)
    end)
    Btn.MouseButton1Click:Connect(function()
        if callback then
            -- Улучшенный ripple с scale и fade
            ...
            ripple.Parent = Btn
            SugarUI.Tween(ripple, {Size = UDim2.new(3, 0, 3, 0), ImageTransparency = 1}, 0.42, Enum.EasingStyle.Quint)
            task.delay(0.42, function() if ripple then ripple:Destroy() end end)
            pcall(callback)
        end
    end)
    self.Instance = Btn
    function self:UpdateTheme()
        ...
        if grad then
            grad.Color = ColorSequence.new(SugarUI.Theme.Button, SugarUI.Theme.ButtonHover)
        end
    end
    return self
end
-- ======================
-- Toggle component (улучшенный: анимация scale, градиент)
-- ======================
...
function ToggleComponent.new(parent, text, default, callback, configKey)
    ...
    stroke.Thickness = 1.2
    SugarUI.AddGradient(Frame, 0, ColorSequence.new(SugarUI.Theme.Panel, SugarUI.Theme.ButtonHover), NumberSequence.new(0.05, 0.15))
    ...
    Box.Parent = Frame
    SugarUI.RoundCorner(12).Parent = Box
    ...
    check.Parent = Box
    SugarUI.AddShadow(Box, 0.55, 8)
    Frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            self.State = not self.State
            SugarUI.Tween(Box, {BackgroundColor3 = self.State and SugarUI.Theme.Accent or SugarUI.Theme.ToggleBox, Size = UDim2.new(0, 26, 0, 26)}, 0.18)
            task.delay(0.18, function() SugarUI.Tween(Box, {Size = UDim2.new(0, 24, 0, 24)}, 0.12) end)
            SugarUI.Tween(check, {ImageTransparency = self.State and 0 or 1}, 0.18)
            check.Visible = true
            task.delay(0.18, function() if not self.State then check.Visible = false end end)
    ...
    self.Set = function(newState, fire)
        ...
        SugarUI.Tween(Box, {BackgroundColor3 = self.State and SugarUI.Theme.Accent or SugarUI.Theme.ToggleBox}, 0.18)
    ...
    function self:UpdateTheme()
        ...
        if grad then
            grad.Color = ColorSequence.new(SugarUI.Theme.Panel, SugarUI.Theme.ButtonHover)
        end
    end
    return self
end
-- ======================
-- Slider component (улучшенный: градиент на fill, smooth drag)
-- ======================
...
function SliderComponent.new(parent, text, min, max, default, callback, configKey)
    ...
    SugarUI.AddGradient(Fill, 90, ColorSequence.new(SugarUI.Theme.AccentDark, SugarUI.Theme.AccentSoft))
    ...
    local function set_value(newValue, fire)
        ...
        SugarUI.Tween(Fill, {Size = UDim2.new(fillSize, 0, 1, 0)}, 0.18)
    ...
    Track.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            SugarUI.Tween(Track, {BackgroundColor3 = Color3.fromRGB(70, 70, 70)}, 0.12)
        end
    end)
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then 
            dragging = false 
            SugarUI.Tween(Track, {BackgroundColor3 = Color3.fromRGB(50, 50, 50)}, 0.12)
        end
    end)
    ...
    function self:UpdateTheme()
        ...
        if grad then
            grad.Color = ColorSequence.new(SugarUI.Theme.AccentDark, SugarUI.Theme.AccentSoft)
        end
    end
    return self
end
-- ======================
-- Dropdown component (улучшенный: анимация открытия с bounce, градиенты на опциях)
-- ======================
...
function DropdownComponent.new(parent, text, options, default, callback, multiSelect, configKey)
    ...
    stroke.Transparency = 0.8
    stroke.Thickness = 1.2
    SugarUI.AddGradient(Frame, 90, ColorSequence.new(SugarUI.Theme.Button, SugarUI.Theme.ButtonHover))
    local Label = Instance.new("TextLabel")
    ...
    OptionsFrame.ScrollBarImageTransparency = 0.4
    ...
    local function create_option(optionText, index)
        ...
        SugarUI.AddGradient(OptionButton, 90, ColorSequence.new(SugarUI.Theme.Panel, SugarUI.Theme.ButtonHover), NumberSequence.new(0.1, 0.2))
        ...
        optionStroke.Transparency = 0.85
        ...
        if multiSelect then
            ...
            OptionButton.MouseButton1Click:Connect(function()
                ...
                for _, obj in ipairs(optionObjects) do
                    SugarUI.Tween(obj.btn, {BackgroundColor3 = (selected == obj.btn.Text) and SugarUI.Theme.AccentSoft or SugarUI.Theme.Panel}, 0.18)
                end
            end)
        end
    ...
    function self:Toggle()
        ...
        if isOpen then
            ...
            local height = math.min((#options * 32 + (multiSelect and 38 or 0) + 10), 240)
            SugarUI.Tween(OptionsFrame, {Size = UDim2.new(1, 0, 0, height)}, 0.28, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out)
            SugarUI.Tween(Frame, {Size = UDim2.new(1, -10, 0, 38 + height)}, 0.28, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out)
            OptionsFrame.ZIndex = 1000
        else
            Label.Visible = true
            ValueLabel.Visible = true
            SugarUI.Tween(OptionsFrame, {Size = UDim2.new(1, 0, 0, 0)}, 0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
            SugarUI.Tween(Frame, {Size = UDim2.new(1, -10, 0, 38)}, 0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
            task.delay(0.24, function() OptionsFrame.ZIndex = 60 end)
        end
    end
    ...
    function self:UpdateTheme()
        ...
        for _, obj in ipairs(optionObjects) do
            ...
            local grad = obj.btn:FindFirstChildOfClass("UIGradient")
            if grad then
                grad.Color = ColorSequence.new(SugarUI.Theme.Panel, SugarUI.Theme.ButtonHover)
            end
        end
...
-- ======================
-- Section component (улучшенный: градиент на линии)
-- ======================
...
function SectionComponent.new(parent, title)
    ...
    line.Position = UDim2.new(0, 12, 1, -8)
    line.BackgroundColor3 = SugarUI.Theme.Border
    line.BorderSizePixel = 0
    line.Parent = wrapper
    SugarUI.AddGradient(line, 0, ColorSequence.new(SugarUI.Theme.Accent, SugarUI.Theme.Border))
    self._wrapper = wrapper
    function self:UpdateTheme()
        label.TextColor3 = SugarUI.Theme.Muted
        line.BackgroundColor3 = SugarUI.Theme.Border
        local grad = line:FindFirstChildOfClass("UIGradient")
        if grad then
            grad.Color = ColorSequence.new(SugarUI.Theme.Accent, SugarUI.Theme.Border)
        end
    end
    return self
end
-- ======================
-- Notifications (улучшенные: анимация входа с стороны, градиент)
-- ======================
...
function NotificationSystem.new(screenGui)
    ...
    self.Container.Position = UDim2.new(1, -360, 0, 30)
...
function NotificationSystem:Notify(title, message, duration, notifType)
    ...
    SugarUI.AddShadow(notification, 0.25, 10)
    SugarUI.AddGradient(notification, 90, ColorSequence.new(SugarUI.Theme.Panel, SugarUI.Theme.Background), NumberSequence.new(0.05, 0.15))
    ...
    local textHeight = 0
    if message then
        local size = TextService:GetTextSize(message, 13, Enum.Font.GothamMedium, Vector2.new(260, 1000))
        textHeight = size.Y
    end
    local totalHeight = math.clamp(60 + textHeight, 64, 150)
    messageLabel.Size = UDim2.new(1, -70, 0, textHeight)
    notification.Position = UDim2.new(1, 50, 0, 0) -- start from right
    SugarUI.Tween(notification, {Position = UDim2.new(1, 0, 0, 0), Size = UDim2.new(1, 0, 0, totalHeight)}, 0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
...
function NotificationSystem:Remove(notification)
    SugarUI.Tween(notification, {Position = UDim2.new(1, 50, 0, 0), Size = UDim2.new(1, 0, 0, 0)}, 0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
    task.delay(0.24, function() if notification.Parent then notification:Destroy() end end)
    for i, notif in ipairs(self.Notifications) do if notif == notification then table.remove(self.Notifications, i); break end end
end
-- ======================
-- Window & Tabs (улучшенный дизайн: градиенты, анимации, современный вид)
-- ======================
local Window = {}
...
local function createTab(selfObj, name)
    ...
    indicator.Position = UDim2.new(0, -8, 0, 5)
    indicator.BackgroundColor3 = SugarUI.Theme.Accent
    indicator.Visible = false
    indicator.BorderSizePixel = 0
    indicator.Parent = tabBtn
    SugarUI.RoundCorner(2.5).Parent = indicator
    SugarUI.AddGradient(indicator, 90, ColorSequence.new(SugarUI.Theme.AccentSoft, SugarUI.Theme.AccentDark))
    ...
    tabBtn.MouseButton1Click:Connect(function()
        ...
        for _, t in ipairs(selfObj.Tabs) do
            ...
            SugarUI.Tween(t.button, {TextColor3 = (t.name == name) and SugarUI.Theme.Text or SugarUI.Theme.Muted}, 0.18)
        end
...
function Window.new(title)
    ...
    OuterFrame.Size = UDim2.new(0, 540, 0, 440)
    OuterFrame.Position = UDim2.new(0.5, -270, 0.5, -220)
    ...
    SugarUI.AddShadow(Frame, 0.25, 10)
    SugarUI.AddGradient(Frame, 90, ColorSequence.new(SugarUI.Theme.Background, SugarUI.Theme.Panel), NumberSequence.new(0.1, 0.2))
    ...
    topGradient.Transparency = NumberSequence.new(0, 0.05)
    ...
    TitleLbl.Size = UDim2.new(0.8, -26, 1, 0)
    ...
    Subtitle.Size = UDim2.new(0.2, -26, 1, 0)
    Subtitle.Position = UDim2.new(0.8, 14, 0, 0)
    ...
    MinimizeBtn.Position = UDim2.new(1, -108, 0.5, -19)
    ...
    MinimizeBtn.MouseEnter:Connect(function() SugarUI.Tween(MinimizeBtn, {BackgroundColor3 = SugarUI.Theme.ButtonHover}, 0.18) end)
    MinimizeBtn.MouseLeave:Connect(function() SugarUI.Tween(MinimizeBtn, {BackgroundColor3 = SugarUI.Theme.Warning}, 0.18) end)
    MinimizeBtn.MouseButton1Click:Connect(function() selfObj:Hide() end)
    local CloseBtn = Instance.new("TextButton")
    CloseBtn.Size = UDim2.new(0, 38, 0, 38)
    CloseBtn.Position = UDim2.new(1, -58, 0.5, -19)
    ...
    CloseBtn.MouseEnter:Connect(function() SugarUI.Tween(CloseBtn, {BackgroundColor3 = SugarUI.Theme.ButtonHover}, 0.18) end)
    CloseBtn.MouseLeave:Connect(function() SugarUI.Tween(CloseBtn, {BackgroundColor3 = SugarUI.Theme.Error}, 0.18) end)
    ...
    tabsPadding.PaddingLeft = UDim.new(0, 14)
    tabsPadding.PaddingRight = UDim.new(0, 14)
    tabsPadding.PaddingBottom = UDim.new(0, 18)
    local PagesHolder = Instance.new("Frame")
    PagesHolder.Size = UDim2.new(1, -190, 1, -58)
    PagesHolder.Position = UDim2.new(0, 190, 0, 58)
    ...
    local function updateOuterSize()
        ...
        local w = math.clamp(math.floor(vp.X * 0.52), 380, 1300)
        local h = math.clamp(math.floor(vp.Y * 0.56), 260, 920)
    ...
    updateOuterSize()
    -- Плавное перетаскивание (улучшенное с quint easing)
    local dragging = false
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            ...
            activeTween = TweenService:Create(OuterFrame, TweenInfo.new(0.08, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = newPos})
            activeTween:Play()
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if resizing and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local newWidth = math.max(380, resizeFrameSize.X.Offset + delta.X)
            local newHeight = math.max(260, resizeFrameSize.Y.Offset + delta.Y)
    ...
    local function createMobileButton(name, sizeX, sizeY, pos)
        ...
        btn.Position = pos or UDim2.new(1, -190, 1, -130)
    ...
    if UserInputService.TouchEnabled then
        local toggleBtn = createMobileButton("GUI", 100, 50, UDim2.new(1, -190, 1, -130))
    ...
    -- Show / Hide (обновленная анимация: scale от 0.8 + fade + bounce)
    function selfObj:Show()
        selfObj.Visible = true
        OuterFrame.Visible = true
        OuterFrame.Scale = 0.8
        OuterFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
        Frame.BackgroundTransparency = 1
        SugarUI.Tween(OuterFrame, {Scale = 1}, 0.32, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out)
        SugarUI.Tween(Frame, {BackgroundTransparency = 0.04}, 0.32, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
    ...
    function selfObj:Hide()
        ...
        SugarUI.Tween(OuterFrame, {Scale = 0.8}, 0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
        SugarUI.Tween(Frame, {BackgroundTransparency = 1}, 0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
        for _, child in ipairs(Notifications.Container:GetChildren()) do
            if child:IsA("Frame") then
                pcall(function() SugarUI.Tween(child, {Size = UDim2.new(1,0,0,0)}, 0.28) end)
            end
        end
    ...
    function selfObj:Confirm(title, msg, yesCb, noCb)
        ...
        panel.Size = UDim2.new(0, 380, 0, 190)
        panel.Position = UDim2.new(0.5, -190, 0.5, -95)
        panel.BackgroundColor3 = SugarUI.Theme.Panel
        SugarUI.RoundCorner(12).Parent = panel
        panel.Parent = overlay
        panel.ZIndex = 1000
        SugarUI.AddShadow(panel, 0.4, 14)
        local stroke = Instance.new("UIStroke", panel)
        stroke.Color = SugarUI.Theme.Border
        stroke.Transparency = 0.75
        local titleLbl = Instance.new("TextLabel")
        titleLbl.Size = UDim2.new(1,-22,0,38)
        titleLbl.Position = UDim2.new(0,11,0,11)
        titleLbl.Text = title
        titleLbl.TextColor3 = SugarUI.Theme.Text
        titleLbl.Font = Enum.Font.GothamBold
        titleLbl.TextSize = 18
        ...
        msgLbl.Size = UDim2.new(1, -22, 0, 85)
        msgLbl.Position = UDim2.new(0,11,0,49)
        msgLbl.Text = msg
        ...
        yesBtn.Instance.Size = UDim2.new(0.42, 0, 0, 38)
        ...
        noBtn.Instance.Size = UDim2.new(0.42, 0, 0, 38)
        noBtn.Instance.Position = UDim2.new(0.5, 0, 1, -52)
