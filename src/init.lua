...
function DropdownComponent.new(parent, text, options, default, callback, multiSelect, configKey)
    ...
    local function create_option(optionText, index)
        ...
        if multiSelect then
            ...
            OptionButton.MouseButton1Click:Connect(function()
                ...
                local isSelected = table.find(selected, optionText) ~= nil
                local check = OptionButton:FindFirstChild("Check")
                local checkIcon = check:FindFirstChild("CheckIcon")
            ...
            OptionButton.MouseButton1Click:Connect(function()
                ...
                for _, obj in ipairs(optionObjects) do
                    SugarUI.Tween(obj.btn, {BackgroundColor3 = (selected == obj.btn.Text) and SugarUI.Theme.AccentSoft or SugarUI.Theme.Button}, 0.12)
                end
            end)
        end
        ...
        OptionButton.MouseLeave:Connect(function()
            local targetColor = multiSelect and SugarUI.Theme.Button or (isSelected and SugarUI.Theme.AccentSoft or SugarUI.Theme.Button)
            SugarUI.Tween(OptionButton, {BackgroundColor3 = targetColor}, 0.1)
        end)

        optionObjects[#optionObjects + 1] = {btn = OptionButton, optionStroke = optionStroke}
    end
    ...
    function self:UpdateOptions(newOptions)
        ...
        for _, obj in ipairs(optionObjects) do
            if obj.btn:FindFirstChild("Check") then
                local check = obj.btn:FindFirstChild("Check")
                local isSelected = table.find(selected, obj.btn.Text) ~= nil
                check.BackgroundColor3 = isSelected and SugarUI.Theme.Accent or SugarUI.Theme.Toggle
                local img = check:FindFirstChild("CheckIcon")
                if img then img.Visible = isSelected end
            else
                SugarUI.Tween(obj.btn, {BackgroundColor3 = (selected == obj.btn.Text) and SugarUI.Theme.AccentSoft or SugarUI.Theme.Button}, 0.12)
            end
        end
    ...
    function self:UpdateTheme()
        ...
        for _, obj in ipairs(optionObjects) do
            ...
            if obj.btn:FindFirstChild("Check") then
                ...
                obj.btn.BackgroundColor3 = (selected == obj.btn.Text) and SugarUI.Theme.AccentSoft or SugarUI.Theme.Button
            end
        end
    end
    return self
